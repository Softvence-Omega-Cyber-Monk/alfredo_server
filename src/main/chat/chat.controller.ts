...
export class ChatController {
  ...
  async getUserChatHistory(@Param('userId') userId: string): Promise<ChatMessage[]> {
  ...
  async getExchangeChatHistory(@Param('exchangeRequestId') exchangeRequestId: string): Promise<ChatMessage[]> {
  ...
  async sendMessage(@Body() body: { senderId: string; receiverId: string; content: string; exchangeRequestId?: string }): Promise<ChatMessage> {
    const savedMessage = await this.chatService.saveMessage(body);

    // Emit via WebSocket to both sender and receiver
    this.chatGateway.server.to(body.receiverId).emit('receive_message', savedMessage);
    this.chatGateway.server.to(body.senderId).emit('receive_message', savedMessage);
  ...
  async getMessages(@Param('userA') userA: string, @Param('userB') userB: string) {
